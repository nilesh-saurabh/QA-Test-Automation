  ...
  return (
    <ThemeProvider theme={theme}>
      <Container maxWidth="md">
        <LoginContainer>
          <LoginFormContainer>
            <FormHeader data-testid="welcome">{`${getWelcome()} ${getName(user.firstName)}`}</FormHeader>
            <CreateNewTodoContainer>
              <IconButton aria-label="delete" onClick={() => history.push('/todo/new')}>
                <AddIcon color="action" fontSize="small" data-testid="add" />
              </IconButton>
              <FormSubHeader>Add a new Todo</FormSubHeader>
            </CreateNewTodoContainer>
            {currentTodos.map((todo) => (
              <TodoItem key={todo._id} todo={todo} updateTodo={setTodos} handleDelete={handleDelete} />
            ))}
            {!currentTodos.length && <h4 data-testid="no-todos">No Available Todos</h4>}
